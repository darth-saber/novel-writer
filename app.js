...
function initializeEventListeners() {
    ...
    document.getElementById('save-chapter').addEventListener('click', saveCurrentChapter);
    document.getElementById('new-chapter').addEventListener('click', createNewChapter);
    
    // Chapters
    document.getElementById('add-chapter-btn').addEventListener('click', () => openChapterModal());
    document.getElementById('close-chapter-modal').addEventListener('click', closeChapterModal);
    document.getElementById('cancel-chapter').addEventListener('click', closeChapterModal);
    document.getElementById('save-chapter-modal').addEventListener('click', saveChapterFromModal);
    
    // Characters
    document.getElementById('add-character-btn').addEventListener('click', () => openCharacterModal());
    document.getElementById('close-character-modal').addEventListener('click', closeCharacterModal);
    document.getElementById('cancel-character').addEventListener('click', closeCharacterModal);
    document.getElementById('save-character-modal').addEventListener('click', saveCharacterFromModal);
    
    // Plot Points
    document.getElementById('add-plot-point-btn').addEventListener('click', () => openPlotModal());
    document.getElementById('close-plot-modal').addEventListener('click', closePlotModal);
    document.getElementById('cancel-plot').addEventListener('click', closePlotModal);
    document.getElementById('save-plot-modal').addEventListener('click', savePlotFromModal);
    ...
    document.getElementById('line-height').addEventListener('change', updateLineHeight);
    ...
    // Chapter title input
    document.getElementById('chapter-title').addEventListener('blur', updateChapterTitle);
}
...
function saveCurrentChapter() {
    ...
    if (AppData.currentChapterId) {
        ...
        const chapter = AppData.chapters.find(c => c.id === AppData.currentChapterId);
        if (chapter) {
            chapter.title = title;
...
function renderChaptersList() {
    ...
    const sortedChapters = [...AppData.chapters].sort((a, b) => a.number - b.number);
...
function openChapterModal(chapterId = null) {
    ...
    if (chapterId) {
        ...
        document.getElementById('chapter-number-input').value = AppData.chapters.length + 1;
...
function saveChapterFromModal() {
    const number = parseInt(document.getElementById('chapter-number-input').value);
...
function renderCharactersGrid() {
    ...
    container.innerHTML = AppData.characters.map(character => `
...
function openCharacterModal(characterId = null) {
    ...
    if (characterId) {
        ...
        if (character) {
            ...
            document.getElementById('character-traits').value = character.traits || '';
...
function saveCharacterFromModal() {
    ...
    if (editingCharacterId) {
        const character = AppData.characters.find(c => c.id === editingCharacterId);
        if (character) {
            character.name = name;
            character.role = role;
            character.bio = bio;
...
function renderPlotTimeline() {
    ...
    // Sort by type order
    const typeOrder = ['exposition', 'rising-action', 'climax', 'falling-action', 'resolution'];
...
function openPlotModal(plotId = null) {
    ...
    if (plotId) {
        ...
        if (point) {
            ...
            document.getElementById('plot-description').value = point.description || '';
        }
    } else {
        editingPlotId = null;
...
function clearAllData() {
    if (confirm('Are you sure you want to delete ALL data? This cannot be undone!')) {
        if (confirm('This will delete all chapters, characters, and plot points. Continue?')) {
            AppData.chapters = [];
            AppData.characters = [];
            AppData.plotPoints = [];
            AppData.currentChapterId = null;
...
function updateStats() {
    document.getElementById('total-chapters').textContent = AppData.chapters.length;
    document.getElementById('total-characters').textContent = AppData.characters.length;
    updateTotalWords();
}
...
function initializeAIAssistant() {
    ...
    document.getElementById('insert-suggestion').addEventListener('click', insertSuggestion);
}
...
function generateContinuation(content, userPrompt) {
    ...
    const lastParagraph = content.split(/[.!?]+/).filter(s => s.trim()).pop()?.trim() || '';
    const selected = continuations[Math.floor(Math.random() * continuations.length)];
...
function generateDialogue(userPrompt) {
    ...
    return dialogueTemplates[Math.floor(Math.random() * dialogueTemplates.length)];
}
...
function insertSuggestion() {
    ...
    editor.value = currentContent.substring(0, startPos) + currentSuggestion + currentContent.substring(endPos);
    
    // Place cursor after inserted text
    editor.focus();
    editor.selectionStart = editor.selectionEnd = startPos + currentSuggestion.length;
